...
'''
#porting to python3 would be too much of a hassle, 
# and would potentially have to add more types that could be 
# tainted?
import inspect
import sys
...
# this is essentially a flexible map that deeply applies
# a function to nested elements, only stopping when the 
# type of o is some base element (see: tclasses, but also most other things)
...
# python2 amirite (hoisting)
# r är result av funktion applicerad med dess argument
# ts är tag set för untrusted
def taint_aware(r, ts=set()):
    r = mapt(r, tclass) #
    # seems to be be useful to essentially create classes of each result?
    update_tags(r, ts)
    return r
...
def ssink(v=None, reached=reached):
    ...
    def _ssink(f):
        def inner(*args, **kwargs):
            #check args before executing f
            allargs = chain(args, kwargs.itervalues())
            if v is None:   # sensitive to ALL
                            # above comment is from original author
                            # horrible name for a variable
                            # v is "senstive tags"
                for a in allargs:
                    t = set()
                    collect_tags(a, t)
...
def taint_class(klass, methods=None):
    ...
    class tklass(klass): 
        def __new__(cls, *args, **kwargs):
            ...
            self.taints = set() #could improve learnability by making this a 
            # set that only can receive the TAGS (because that's what it does)
            ...
            for a in args:      # this CHUNK of code appears at least 3 times, refactor later
                                # above comment was added by original authors, could just create a helper? 
                                # allargs = chain(args, kwargs.itervalues()) is used in ssink
    ...
    # update the tklass constructor with extra methods if needed
    d = klass.__dict__
...
UNICODE = taint_class(unicode, unicode_methods) #unicode is type in python2
...
def tclass(o):
    ...
    if klass in tclasses.keys():
        ...
        #      ^              ^
        #      | "tklass"     | construct tklass
        # in this case would first be
        # an object of "tklass" that has the attributes of the previous class
        #   and taints? (haven't checked propagate method)
